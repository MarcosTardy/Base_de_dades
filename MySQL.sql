...
select * from movies.tb_person;
select * from movies.tb_role;
select * from movies.tb_movie_person;
...
select person_name, person_country, person_dob from movies.tb_person where person_dod is null order by person_dob;
select genre_name, count(movie_id) from tb_genre inner join tb_movie on tb_genre.genre_id = tb_movie.movie_genre_id group by genre_name order by count(movie_id) desc;
